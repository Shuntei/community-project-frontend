...
export default function SeeMoreFollows({ marginTop = `` }) {
  ...
  const fetchAFollowers = () => {
    fetch(`${SN_SHOW_FOLLOWS}${location.search}`)
      .then((r) => r.json())
      .then((result) => {
        console.log(result)
        result.followers && setFollowersInfo(result.followers)
      })
  }

  const allUsersSearchHandler = () => {
    // e.preventDefault()
      ...
      <div className="flex justify-center pb-5 px-10 mb-5 rounded-b-lg bg-neutral-300">
        <div className="flex-col items-center">
          <div className="text-center py-3 text-[20px]">
            {showAllUsers ? 'ALL USERS' : 'FOLLOWERS'}
          </div>
          <div className="flex justify-center py-5">
            ...
            <span
              // onChange={allUsersSearchHandler}
