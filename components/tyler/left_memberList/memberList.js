...
export default function MemberList({ totalBonus }) {
  ...
  return (
    <div
      className={`${styles['sidebar']} ${showSidebar ? '' : styles.hidden_left} ${!onPhone ? '' : showMember ? styles.show_up : styles.hidden_down}`}
    >
      {!onPhone ? (
        ''
      ) : !showMember ? (
      ...
      <div className={`${styles['sidebar-container']}`}>
        <div className={`text-left mt-5 mb-1 max-md:mt-0 `}>
          直播總打賞: {totalBonus}
        </div>
